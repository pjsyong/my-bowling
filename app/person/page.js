...
export default function IntegratedRecordPage() {
  ...
  const { data: rankings = [], isLoading: isRankingsLoading } = useQuery({
    ...
    queryFn: async () => {
      // user_public에서 type_pro와 official 컬럼 추가 추출
      const { data: entryData } = await supabase
        .from('entry')
        .select('*, user:user_public!user_id(name, type_pro, official)')
    ...
    <div className="max-w-md mx-auto pt-16">
      <div className="mb-8 px-2">
        ...
        <div className="relative">
          <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center gap-2 text-slate-500 font-bold text-sm">
      ...
      <div className="flex bg-slate-100 p-1.5 rounded-2xl mb-6">
        <button onClick={() => setActiveTab('ranking')} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-[14px] text-sm font-bold transition-all ${activeTab === 'ranking' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}><ListOrdered size={16} /> 리더보드</button>
        <button onClick={() => setActiveTab('prize')} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-[14px] text-sm font-bold transition-all ${activeTab === 'prize' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}><Wallet size={16} /> 상금 현황</button>
      </div>

      <AnimatePresence mode="wait">
        {!selectedEvent?.progress ? (
          <motion.div key="not-started" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-20 text-center">

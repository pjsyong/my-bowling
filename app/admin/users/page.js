...
import { UserPlus, Pencil, Trash2, ChevronLeft, ArrowLeft, X } from 'lucide-react';
...
export default function UserManagementPage() {
  ...
  const [formData, setFormData] = useState({ name: '', phone: '', type_pro: 0 });
  ...
  const fetchUsers = async () => {
    try {
      const { data, error } = await supabase
        .from('user')
        .select('*')
        .order('user_id', { ascending: true }); // ID 순으로 정렬
  ...
  useEffect(() => {
    ...
    if (authStatus !== 'true') {
      alert('관리자 인증이 필요합니다.');
      router.push('/admin'); // 인증 없으면 로그인 페이지로 이동
    }
      ...
      if (editingUser) {
        ...
        const { error } = await supabase
          ...
          .update({
            ...
            type_pro: parseInt(formData.type_pro) // 숫자로 변환
          })
        ...
        const { error } = await supabase
          ...
          .insert([{
            ...
            created_at: today // 현재 DB 컬럼이 text이므로 날짜 직접 주입
          }]);
    ...
    <div className="flex flex-col gap-6 mb-10">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1 font-medium">클럽 멤버들의 정보를 수정하거나 삭제할 수 있습니다.</p>
        </div>
        
        <button 
          onClick={() => { setEditingUser(null); setFormData({ name: '', phone: '', type_pro: 0 }); setIsModalOpen(true); }}
          ...
          <tbody className="divide-y divide-slate-50">
            {users.map((user) => (
              <tr key={user.user_id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-8 py-6 text-slate-400 text-xs font-medium">{user.created_at}</td>
                <td className="px-8 py-6 text-right">
                  <div className="flex justify-end gap-2">
                    <button onClick={() => { setEditingUser(user); setFormData({ name: user.name, phone: user.phone, type_pro: user.type_pro }); setIsModalOpen(true); }} className="p-2 text-slate-300 hover:text-blue-500 transition-colors">
                      <Pencil size={18} />
                    </button>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3 mt-2">
                  <button type="button" onClick={() => setFormData({...formData, type_pro: 0})} className={`py-4 rounded-2xl font-black text-sm transition-all ${formData.type_pro === 0 ? 'bg-slate-900 text-white shadow-lg' : 'bg-slate-50 text-slate-400'}`}>일반</button>
                  <button type="button" onClick={() => setFormData({...formData, type_pro: 1})} className={`py-4 rounded-2xl font-black text-sm transition-all ${formData.type_pro === 1 ? 'bg-blue-600 text-white shadow-lg' : 'bg-slate-50 text-slate-400'}`}>프로</button>
                </div>
              </div>
